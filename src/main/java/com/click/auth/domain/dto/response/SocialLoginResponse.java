...
public record SocialLoginResponse(
    ...
    public static SocialLoginResponse from(KakaoUserInfoResponse res, boolean isAlready) {
        return new SocialLoginResponse(
            ...
            res.id().toString(),
            UserIdentityType.KAKAO,
            res.kakao_account().profile().is_default_nickname()
                ? null : res.kakao_account().profile().nickname(),
            res.kakao_account().profile().is_default_image()
                ? null : res.kakao_account().profile().profile_image_url()
        );
    }
}
